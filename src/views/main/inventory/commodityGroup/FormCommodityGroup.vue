...
import { BaseInput, BaseCombobox } from '@/core/public_component';
...
const callApiForm = async (api: any, stateForm: string = '') => {
  ...
  await props.Base.apiService.callApi(api, commodityGroup.value ,async (response: any) => { 
    ...
    if(res.data.userMsg === 'validate.unique') {
      props.Base.addNotification(ENotificationType.Error, t(res.data.userMsg, {field: t("module.inventory.commodityCode"), value: commodityGroup.value.commodityCode}))
    }
    else{
